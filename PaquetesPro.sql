...
Este script contiene los siguientes paquetes para:
    >>> Country                    
    >>> District                   
    >>> EMAIL                      
    >>> Log_in                     
...
create or replace package packageCountry is
    ...
    procedure updateCountry(pnIdCountry number, pvNewName varchar2);
...
create or replace package body packageCountry as
    ...
    procedure updateCountryName(pnIdCountry number, pvNewName varchar2) as
        ...
        begin 
            update counntry set name = pvNewName where id_country = pnIdCountry;
            commit;
...
create or replace package packageDistrict is
    ...
    procedure updateDistrict(pnIdDistrict number, pvNewName varchar2);
...
create or replace package packagePerson is
    ...
    procedure createPerson(pvName varchar2, pnValue number);
...
create or replace package body packagePerson is
    ...
    function getPersonName(pnIdPerson number) return varchar2 is
        vcName varchar2(100);
...
create or replace package body packagePersonPhoto is
   ...
   procedure createPersonPhoto(pvContent varchar2, pnIdPerson number) as
    ...
    procedure updatePersonPhotoContent(pnIdPersonPhoto number, pvNewContent varchar2) as

        begin 
            update person_photo set content = pvNewContent where id_person_photo = pnIdPersonPhoto;
            commit;

        end updatePersonPhotoContent;
...
create or replace package packagePersonxCategory is
    ...
    procedure deletePersonPhoto(pnIdPerson number, pnIdCategory number);
